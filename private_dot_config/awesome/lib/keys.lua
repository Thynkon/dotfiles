...
-- local my_show_help = awful.widget.hotkeys_popup.widget({
-- 	-- bg = "#ff0000",
-- 	font = "Sans 16",
-- })
...
local root_keys = table.join(
	...
	awful.key({ settings.modkey }, "s", hotkeys_popup.show_help, { description = "Cheat sheet", group = "awesome" }),
	-- awful.key({ settings.modkey }, "s", function()
	-- 	my_show_help.show_help()
	-- end, { description = "Cheat sheet", group = "awesome" }),
	-- Show/Hide Systray
	awful.key({ settings.modkey }, "KP_Subtract", function()
	...
	awful.key({ settings.superkey }, "x", function()
		awful.prompt.run({
			prompt = "Run Lua code: ",
	...
	end, { description = settings.terminal, group = "launcher" }),

	awful.key({ settings.modkey, "Shift" }, "h", function()
		awful.spawn(string.format("%s -t 'htop task manager' -e htop", settings.terminal))
	end, { description = "htop", group = "launcher" }),

	awful.key({ settings.modkey }, "v", function()
		awful.spawn("pavucontrol")
	end, { description = "pulseaudio control", group = "launcher" }),

	awful.key({ settings.modkey }, "w", function()
		awful.spawn(settings.browser)
	end, { description = settings.browser, group = "launcher" }),

	awful.key({ settings.modkey, "Shift" }, "e", function()
		awful.spawn(settings.editorgui)
	end, { description = settings.editorgui, group = "launcher" }),

	awful.key({ settings.modkey }, "F3", function()
	...
	end, { description = settings.imageeditor, group = "launcher" }),

	awful.key({ settings.modkey }, "e", function()
		awful.spawn(settings.filemanager)
	end, { description = settings.filemanager, group = "launcher" }),

	awful.key({ settings.modkey }, "F9", function()
		awful.spawn(settings.mailclient)
	end, { description = settings.mailclient, group = "launcher" }),

	awful.key({ settings.modkey }, "d", function()
		awful.spawn(string.format("%s/.config/rofi/launchers/%s/launcher.sh", os.getenv("HOME"), settings.rofi_theme))
	end, { description = "rofi", group = "launcher" }),

	awful.key({ settings.modkey }, "y", function()
		awful.spawn(string.format("%s/.local/bin/udiskie-dmenu", os.getenv("HOME")))
	end, { description = "Device manager", group = "launcher" }),

	-- super + ...
	awful.key({ settings.modkey }, "u", function()
	...
	awful.key({ settings.modkey }, "Escape", function()
		awful.spawn("xkill")
	end, { description = "Kill proces", group = "hotkeys" }),

	-- ctrl+alt +  ...
	awful.key({ settings.ctrlkey, settings.superkey }, "o", function()
		awful.spawn.with_shell("$HOME/.config/awesome/scripts/compton-toggle.sh")
	end, { description = "Compton toggle", group = "alt+ctrl" }),

	awful.key({ settings.ctrlkey, settings.superkey }, "m", function()
		awful.spawn("systemsettings5")
	end, { description = string.format("%s settings manager", settings.desktopenvironment), group = "alt+ctrl" }),

	-- screenshots
	awful.key({}, "Print", function()
		awful.spawn(settings.screenshotutility)
	end, { description = "Screenshot", group = "screenshots" }),
	...
	awful.key({ settings.modkey }, "é", function()
		awful.client.focus.global_bydirection("right")
	end, { description = "Focus right", group = "client" }),

	-- Layout manipulation
	awful.key({ settings.modkey, "Shift" }, "é", function()
	...
	awful.key({ settings.modkey, "Control" }, "é", function()
	...
	awful.key({ settings.modkey, "Control" }, "+", function()
	...
	awful.key({ settings.modkey, "Shift" }, "r", function()
	...
	end, { description = string.format("+%d%%", settings.brightness_step), group = "hotkeys" }),
	...
	end, { description = string.format("+%d%%", settings.volume_step), group = "audio" }),
	...
	end, { description = "select previous", group = "audio" }),

	awful.key({}, "XF86AudioNext", function()
		os.execute("playerctl --all-players next")
	end, { description = "select next", group = "audio" }),

	awful.key({}, "XF86AudioPlay", function()
		os.execute("playerctl --all-players play-pause")
	end, { description = "toggle", group = "audio" }),

	-- Copy primary to clipboard (terminals to gtk)
	awful.key({ settings.ctrlkey, "Shift" }, "c", function()
		awful.spawn.with_shell("xsel | xsel -i -b")
	end, { description = "copy terminal to gtk", group = "hotkeys" }),

	awful.key({ settings.modkey, "Shift" }, "q", function()
		awful.spawn(string.format("%s/bin/menu_powermenu.sh", settings.rofi_config))
	end, { description = "Logout with confirmation", group = settings.desktopenvironment }),

	awful.key({ settings.ctrlkey, settings.superkey }, "k", function()
		os.execute("loginctl lock-session")
	end, { description = "Lock screen", group = settings.desktopenvironment })
)
