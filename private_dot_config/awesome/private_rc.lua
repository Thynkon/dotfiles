...
awful.util.taglist_buttons = awful.util.table.join(
	...
	awful.button({ keys.root.modkey }, 1, function(t)
		if client.focus then
			client.focus:move_to_tag(t)
		end
	end),
	awful.button({}, 3, awful.tag.viewtoggle),
	awful.button({ keys.root.modkey }, 3, function(t)
		if client.focus then
...
for i = 1, 6 do
	...
	keys.root = table.join(
		keys.root,
		-- View tag only.
		awful.key({ settings.modkey }, np_keys[i], function()
		...
		-- Toggle tag display.
		awful.key({ settings.modkey, "Control" }, np_keys[i], function()
			local screen = awful.screen.focused()
			local tag = screen.tags[i]
			if tag then
		...
		-- Move client to tag.
		awful.key({ settings.modkey, "Shift" }, np_keys[i], function()
			if client.focus then
		...
		awful.key({ settings.modkey, "Control", "Shift" }, np_keys[i], function()
			if client.focus then
				local tag = client.focus.screen.tags[i]
				if tag then
